...
// SeedBuiltinResourceModels inserts the builtin models and their published
// initial version for one organization. The same deterministic inserts run as
// baseline migration 0012 for organizations that already exist at migration
// time; bootstrap runs this for the organization it creates.
func SeedBuiltinResourceModels(ctx context.Context, s *Store, organizationID string) error {
	if s == nil || s.Pool == nil {
	...
	if _, err := s.Pool.Exec(ctx, `
		...
		SELECT $1::uuid, d.model_key, d.name, d.description, d.content_kind, 'active', a.id
		FROM LATERAL (
			SELECT u.id
	...
	`, organizationID); err != nil {
		return fmt.Errorf("seed builtin resource models: %w", err)
	}
	if _, err := s.Pool.Exec(ctx, `
		...
		SELECT m.id, m.organization_id, 1, 'published',
		       d.field_schema, d.form_schema, d.list_schema, d.policy, 'seed:v1', m.created_by, now(), now()
		FROM model.resource_models m
		JOIN (VALUES
			('builtin_document', '{"additional_properties": false, "fields": []}'::jsonb,
			 '{"sections": []}'::jsonb,
			 '{"columns": ["title", "updated_at"], "filters": []}'::jsonb,
			 $policy${
			...
			('builtin_note', '{"additional_properties": false, "fields": []}'::jsonb,
			 '{"sections": []}'::jsonb,
			 '{"columns": ["title", "updated_at"], "filters": []}'::jsonb,
			 $policy${
			   "visibility": {"default": "workspace", "allowed": ["workspace", "organization", "public"]},
			...
			('builtin_faq', '{"additional_properties": false, "fields": []}'::jsonb,
			 '{"sections": []}'::jsonb,
			 '{"columns": ["title", "updated_at"], "filters": []}'::jsonb,
			 $policy${
			   "visibility": {"default": "workspace", "allowed": ["workspace", "organization", "public"]},
			...
			('builtin_shot',
			 ...
			 '{"columns": ["shot_size", "camera_angle", "lens_mm", "location"], "filters": ["shot_size", "camera_angle"]}'::jsonb,
			 $policy${
			   "visibility": {"default": "workspace", "allowed": ["workspace", "organization", "public"]},
			   "channels": {
			     "workspace": {"enabled": true},
	...
	`, organizationID); err != nil {
		return fmt.Errorf("seed builtin model versions: %w", err)
	}
	if _, err := s.Pool.Exec(ctx, `
	...
	`, organizationID); err != nil {
		return fmt.Errorf("link builtin model versions: %w", err)
	}
	return nil
}
