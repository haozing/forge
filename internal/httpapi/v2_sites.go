...
// v2_sites.go — the phase 5 public-site management surface: workspace site
...
// v2SiteError maps site domain errors onto the v2 status/code contract.
...
func v2SiteError(w http.ResponseWriter, err error, conflictCode string) {
	switch {
	case err == nil:
		return
...
type v2CreateSiteRequest struct {
...
type v2UpdateSiteRequest struct {
	Name                *string          `json:"name"`
	Domain              *string          `json:"domain"`
	Template            *string          `json:"template"`
	DefaultContentScope *string          `json:"default_content_scope"`
...
// v2SitesCollection serves GET/POST /api/v2/workspaces/{workspaceId}/sites.
// Reads need site.read; creating needs site.manage plus an Idempotency-Key.
func v2SitesCollection(deps Dependencies) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		case http.MethodPost:
			if !requireWorkspaceActionV2(w, r, deps, principal, workspaceID, authz.ActionSiteManage) {
				return
			}
			if _, ok := requireIdempotencyKeyV2(w, r); !ok {
				return
			}
			var input v2CreateSiteRequest
			if !decodeV2Body(w, r, &input, 256*1024) {
				return
			}
...
// v2SiteResource serves GET/PATCH/DELETE
// /api/v2/workspaces/{workspaceId}/sites/{siteId}. PATCH demands the site
// revision If-Match; DELETE is the soft disable (status='disabled') and
// honors an optional If-Match.
func v2SiteResource(deps Dependencies) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		principal, ok := v2Principal(w, r, deps)
		if !ok {
			return
		...
		case http.MethodPatch:
			...
			if _, ok := requireIfMatchV2(w, r); !ok {
				return
			}
			var input v2UpdateSiteRequest
			if !decodeV2Body(w, r, &input, 256*1024) {
				return
			}
...
type v2CreateBindingRequest struct {
...
type v2UpdateBindingRequest struct {
	DisplayPath   *string          `json:"display_path"`
	ContentType   *string          `json:"content_type"`
	SectionSlug   *string          `json:"section_slug"`
	SortOrder     *int             `json:"sort_order"`
...
// v2SiteBindingsCollection serves GET/POST
// /api/v2/workspaces/{workspaceId}/sites/{siteId}/bindings. The stage 5
// matrix gates both surfaces behind site.manage.
func v2SiteBindingsCollection(deps Dependencies) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		principal, ok := v2Principal(w, r, deps)
		if !ok {
			return
		...
		case http.MethodGet:
			page, err := deps.Sites.ListBindings(r.Context(), principal, workspaceID, siteID,
				r.URL.Query().Get("cursor"), atoiDefault(r.URL.Query().Get("limit"), 50))
			if err != nil {
				v2SiteError(w, err, "path_conflict")
				return
			}
			writeData(w, r, http.StatusOK, map[string]any{
				"items": page.Items,
		...
		case http.MethodPost:
			...
			var input v2CreateBindingRequest
...
// v2SiteBindingResource serves PATCH/DELETE
// /api/v2/workspaces/{workspaceId}/sites/{siteId}/bindings/{bindingId}.
// Bindings carry no revision column, so these commands use the
// Idempotency-Key contract without If-Match.
func v2SiteBindingResource(deps Dependencies) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		principal, ok := v2Principal(w, r, deps)
		if !ok {
			return
		...
		case http.MethodPatch:
			...
			var input v2UpdateBindingRequest
			if !decodeV2Body(w, r, &input, 64*1024) {
				return
			}
...
// v2SitePreview serves GET
// /api/v2/workspaces/{workspaceId}/sites/{siteId}/preview: the JSON snapshot
...
func v2SitePreview(deps Dependencies) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		principal, ok := v2Principal(w, r, deps)
		if !ok {
			return
