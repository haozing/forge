...
func builtinSpecs(handlers BuiltinHandlers) []builtinSpec {
	...
	return []builtinSpec{
		...
		{name: "create_internal_asset", description: "Create an internal draft asset. For document/note models pass title+markdown (fields may be empty); for record models pass fields (title optional). Returns current_working_version_id — pass it as expected_version_id to update_internal_asset", risk: LowWrite, capabilities: []string{"asset.write"}, handler: handlers.CreateInternalAsset, parameters: map[string]*schema.ParameterInfo{"resource_model_id": id("Resource model ID"), "title": {Type: schema.String, Desc: "Asset title (recommended for document/note)"}, "markdown": {Type: schema.String, Desc: "Body markdown (document/note models)"}, "fields": {Type: schema.Object, Desc: "Field values per the model's field schema (record models; empty object for documents)"}, "tag_ids": {Type: schema.Array, Desc: "Existing workspace tag IDs to attach", ElemInfo: &schema.ParameterInfo{Type: schema.String}}}},
		{name: "update_internal_asset", description: "Update an internal draft asset", risk: LowWrite, capabilities: []string{"asset.write"}, handler: handlers.UpdateInternalAsset, parameters: map[string]*schema.ParameterInfo{"asset_id": id("Asset ID"), "expected_version_id": id("Optimistic lock: the current_working_version_id returned by create_internal_asset, or the asset's numeric draft_revision"), "title": {Type: schema.String}, "markdown": {Type: schema.String}, "fields": {Type: schema.Object}, "tag_ids": {Type: schema.Array, Desc: "Replacement tag ID selection (omitted keeps tags, empty clears)", ElemInfo: &schema.ParameterInfo{Type: schema.String}}}},
		{name: "create_relation", description: "Propose a relation between authorized internal assets; it parks on the source draft and materializes at commit. relation_type must be one of: related_to | references | derived_from | cites | continues_from", risk: LowWrite, capabilities: []string{"asset.write"}, handler: handlers.CreateRelation, parameters: map[string]*schema.ParameterInfo{"source_asset_id": id("Source asset ID"), "target_asset_id": id("Target asset ID"), "relation_type": id("related_to | references | derived_from | cites | continues_from")}},
		...
		// publish_asset 已下线（统一方案 J）：asset.publish 是 human_only 动作，
		// agent 只能起草与确认，发布由人在管理面执行。
