...
-- R3 is the canonical retrieval schema. Projection data is derived from
-- immutable asset versions.
DROP TABLE IF EXISTS retrieval.search_session_items CASCADE;
DROP TABLE IF EXISTS retrieval.search_sessions CASCADE;
DROP TABLE IF EXISTS retrieval.chunk_embeddings CASCADE;
DROP TABLE IF EXISTS retrieval.chunks CASCADE;
DROP TABLE IF EXISTS retrieval.projection_runs CASCADE;
DROP TABLE IF EXISTS retrieval.projection_configs CASCADE;
DROP TABLE IF EXISTS retrieval.fulltext_documents CASCADE;
DROP TABLE IF EXISTS retrieval.retrieval_projections CASCADE;
...
BEGIN
  ...
  LOOP
    field_value := fields -> (predicate ->> 'field');
    ...
    CASE op
      ...
      WHEN 'in' THEN
        IF NOT EXISTS (SELECT 1 FROM jsonb_array_elements(expected) item WHERE item = field_value) THEN
          RETURN false;
        END IF;
      WHEN 'gte' THEN
        IF jsonb_typeof(field_value) <> 'number' OR jsonb_typeof(expected) <> 'number'
           OR (field_value #>> '{}')::numeric < (expected #>> '{}')::numeric THEN
          RETURN false;
        END IF;
      WHEN 'lte' THEN
        IF jsonb_typeof(field_value) <> 'number' OR jsonb_typeof(expected) <> 'number'
...
CREATE TABLE retrieval.projection_configs (
    ...
    resource_model_id uuid NOT NULL REFERENCES model.resource_models(id) ON DELETE CASCADE,
    model_name text,
    model_version text,
...
CREATE TABLE retrieval.search_sessions (
    ...
    mode text NOT NULL DEFAULT 'hybrid' CHECK (mode IN ('lexical', 'semantic', 'hybrid')),
    degraded boolean NOT NULL DEFAULT false,
    ranking_method text NOT NULL DEFAULT 'rrf' CHECK (ranking_method IN ('lexical', 'vector', 'rrf', 'rerank')),
